...
- **Auto-results are currently BROKEN — diagnosed 11 Jul 2026, fix pending.** Two independent API-shape bugs in `tennis_auto_results.py`: (1) the fixtures-by-date endpoint paginates (`{"data": [10], "hasNextPage": true}`, e.g. 130 ATP fixtures = 13 pages on 10 Jul) and `_data_list` reads only page 1, so most picks are never found; (2) that endpoint **never returns a `result` field at all** (260/260 finished fixtures checked had none — it is schedule-only), so even found fixtures always look 'not finished'. Results actually live on `/player/past-matches/{playerId}` (`result: "6-1 6-0"`, `match_winner`, `result_type`). The 30-min `tennis_live_results_check` job itself is registered and firing fine; no Sheets write errors — writes are simply never reached. NOTE: the page-1-only fetch also narrows the daily picks job's fixture pool to ~10 fixtures/tour/day.
...
| Area | Done | Status |
|---|---|---|
| Tennis bot core | 90% | Picks, Sheets tab, Discord-only delivery (`tennis-picks`/`tennis-results`, 10 Jul 2026 — no Telegram), CLV polling, daily picks PNG card + simulated Kelly staking/bankroll (11 Jul 2026) live; auto-results shipped but broken (see limitations) |
