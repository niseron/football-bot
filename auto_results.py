...
def evaluate_pick(
    ...
    *,
    ...
    Knockout Match Winner picks carry a time-scope suffix since 12 Jul 2026:
    ...
    scope settles. An unscoped Match Winner follows the bookmaker default:
    90 minutes only.
    """
    bt    = bet_type.lower()
    pk    = pick.lower().strip()
    ...
    if any(x in bt for x in ("match winner", "1x2", "result", "moneyline")):
        ...
        if (extra_time or penalties) and scope != "ft":
